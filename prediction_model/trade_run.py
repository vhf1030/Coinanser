...
while True:
    ...
    for market in market_all:
        if market not in market_status:  # 예측 시작
            ...
            print(datetime_convert(datetime.now()), 'bid_start:', market, bid_price, '( predict:', pm, ')')
        ...
        if market in market_status and market_status[market]['run_state'] == 'run_ask':
            ...
            if market_status[market]['ask_view']['state'] != 'done':
                ...
                if pm and pm < -0.01:  # 주문 취소 후 시장가 매도
                    ...
                    if 'error' not in delete_view:
                        print(datetime_convert(datetime.now()), 'ask_cancel:', market, '( predict:', pm, ')')
            ...
            # DB insert
            converted_value = convert_result(market_status[market])
